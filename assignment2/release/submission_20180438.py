...
def get_conditional_prob1(delta, epsilon, eta, c2, d2):
    ...
    p_c0 = (delta * (1 - epsilon) + (1 - delta) * epsilon)
    p_c1 = (delta * epsilon + (1 - delta) * (1 - epsilon))
    right_sensor = 1 - eta
    wrong_sensor = eta

    if c2 == 0:
        if c2 == d2:
            return p_c0 * right_sensor / (p_c0 * right_sensor + p_c1 * wrong_sensor)
        else:
            return p_c0 * wrong_sensor / (p_c0 * wrong_sensor + p_c1 * right_sensor)
    else:
        if c2 == d2:
            return p_c1 * right_sensor / (p_c1 * right_sensor + p_c0 * wrong_sensor)
        else:
            return p_c1 * wrong_sensor / (p_c1 * wrong_sensor + p_c0 * right_sensor)
    # END_YOUR_ANSWER
...
def get_conditional_prob2(delta, epsilon, eta, c2, d2, d3):
    ...
    p_c2_d3_same = (epsilon * eta + (1 - epsilon) * (1 - eta))
    p_c2_d3_diff = (epsilon * (1 - eta) + (1 - epsilon) * eta)

    c0_d0_d0 = p_c0 * (1 - eta) * p_c2_d3_same
    c0_d0_d1 = p_c0 * (1 - eta) * p_c2_d3_diff
    c0_d1_d0 = p_c0 * eta * p_c2_d3_same
    c0_d1_d1 = p_c0 * eta * p_c2_d3_diff

    c1_d0_d0 = p_c1 * eta * p_c2_d3_diff
    c1_d0_d1 = p_c1 * eta * p_c2_d3_same
    c1_d1_d0 = p_c1 * (1 - eta) * p_c2_d3_diff
    c1_d1_d1 = p_c1 * (1 - eta) * p_c2_d3_same

    if c2 == 0 and d2 == 0 and d3 == 0:
        return c0_d0_d0 / (c0_d0_d0 + c1_d0_d0)
    elif c2 == 0 and d2 == 0 and d3 == 1:
        return c0_d0_d1 / (c0_d0_d1 + c1_d0_d1)
    elif c2 == 0 and d2 == 1 and d3 == 0:
        return c0_d1_d0 / (c0_d1_d0 + c1_d1_d0)
    elif c2 == 0 and d2 == 1 and d3 == 1:
        return c0_d1_d1 / (c0_d1_d1 + c1_d1_d1)
    elif c2 == 1 and d2 == 0 and d3 == 0:
        return c1_d0_d0 / (c1_d0_d0 + c0_d0_d0)
    elif c2 == 1 and d2 == 0 and d3 == 1:
        return c1_d0_d1 / (c1_d0_d1 + c0_d0_d1)
    elif c2 == 1 and d2 == 1 and d3 == 0:
        return c1_d1_d0 / (c0_d1_d0 + c1_d1_d0)
    elif c2 == 1 and d2 == 1 and d3 == 1:
        return c1_d1_d1 / (c0_d1_d1 + c1_d1_d1)
    # END_YOUR_ANSWER
...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.getNumRows()):
            for col in range(self.belief.getNumCols()):
                mean = math.sqrt(math.pow(util.rowToY(row)-agentY, 2) + math.pow(util.colToX(col)-agentX, 2))
                probability_density = util.pdf(mean, Const.SONAR_STD, observedDist)
                self.belief.setProb(row, col, self.belief.getProb(row, col) * probability_density)
    ...
    def elapseTime(self):
        ...
        for oldTile, newTile in self.transProb:
            newProbability = self.belief.getProb(oldTile[0], oldTile[1])*self.transProb[(oldTile, newTile)]
            newBelief.addProb(newTile[0], newTile[1], newProbability)
        newBelief.normalize()
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        observedParticle = collections.defaultdict(int)
        for tile in self.particles:
            mean = math.sqrt(math.pow(util.rowToY(tile[0]) - agentY, 2) + math.pow(util.colToX(tile[1]) - agentX, 2))
            probability_density = util.pdf(mean, Const.SONAR_STD, observedDist)
            observedParticle[tile] = self.particles[tile] * probability_density
        newParticles = collections.defaultdict(int)
        for i in range(self.NUM_PARTICLES):
            particleTile = util.weightedRandomChoice(observedParticle)
            newParticles[particleTile] += 1
    ...
    def elapseTime(self):
        ...
        for tile in self.particles:
            ...
            for i in range(value):
                transParticle = self.transProbDict[tile]
                particleTile = util.weightedRandomChoice(transParticle)
                newParticles[particleTile] += 1
        self.particles = newParticles
        # END_YOUR_ANSWER
